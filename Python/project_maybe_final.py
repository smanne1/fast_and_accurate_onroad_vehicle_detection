...
npi=np.pi/180
NumRows = 180
images = [cv2.imread(file) for file in glob.glob("S:\CNN\Matlab\data24\/*.jpg")]
for frame in images:
    print(time.time()-start)
##    test_count=test_count+1
    print(test_count)
    imlow1=frame[340:700,200:900,:]
...
##    print(np.array([[x1,y1],[x2,y2],[x11,y11],[x22,y22]]))
    ...
    th3 = cv2.Canny(np.uint8(imlowf1*255),150,255,apertureSize = 5)
    th2 = cv2.Canny(np.uint8(masked_image*255),35,100,apertureSize = 3)
    th3=mask2*th3
    kernel = cv2.getStructuringElement(cv2.MORPH_CROSS,(30,5))
